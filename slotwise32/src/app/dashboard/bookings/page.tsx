...
export default async function BookingsPage() {
  ...
  function BookingRow({ b }: { b: any }) {
    ...
    return (
      <div className="px-5 py-4 flex items-center justify-between hover:bg-zinc-900">
        <div className="flex items-center gap-3 min-w-0">
          <div className={`shrink-0 w-2 h-2 rounded-full ${isPast ? 'bg-gray-300' : 'bg-green-500'}`} />
          <div className="min-w-0">
            <p className="text-sm font-medium text-white truncate">{b.booker_name}</p>
        ...
        <div className="flex items-center gap-5 shrink-0 ml-4">
          ...
          <p className="text-xs text-zinc-600 w-28 truncate">{b.booker_email}</p>
        </div>
      </div>
    )
  }
  ...
  function Section({ title, items, empty }: { title: string; items: any[]; empty: string }) {
    return (
      <div className="bg-zinc-950 rounded-xl border border-zinc-800 mb-6">
        <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
          <h2 className="font-medium text-white">{title}</h2>
          <span className="text-xs text-zinc-600 bg-zinc-800 px-2 py-0.5 rounded-full">{items.length}</span>
        </div>
        {items.length > 0 ? (
          ...
          <div className="px-5 py-8 text-center text-sm text-zinc-600">{empty}</div>
        )}
      </div>
    )
  }
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-zinc-500 mt-1">{bookings?.length ?? 0} total across all clients</p>
      </div>
