...
export default function EditLinkPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return (
    <div className="p-8 flex items-center gap-2 text-sm text-zinc-600">
  ...
  return (
    <div className="p-8 max-w-2xl">
      <Link href="/dashboard/links" className="flex items-center gap-1.5 text-sm text-zinc-500 hover:text-white mb-6">
        <ArrowLeft size={15} /> Back to links
      </Link>

      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-white">Edit booking link</h1>
          <p className="text-sm text-zinc-500 mt-0.5">Client: {clientName}</p>
        </div>
      ...
      <form onSubmit={handleSave} className="space-y-5">
        ...
        <div className="bg-zinc-950 rounded-xl border border-zinc-800 p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-white">Link active</p>
              <p className="text-xs text-zinc-500 mt-0.5">Inactive links show a "not available" page</p>
            </div>
            <button
              type="button"
            ...
            >
              <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-zinc-950 rounded-full shadow transition-transform ${form.is_active ? 'translate-x-5' : ''}`} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-zinc-950 rounded-xl border border-zinc-800 p-5 space-y-4">
          <h2 className="text-sm font-semibold text-zinc-300 uppercase tracking-wide">Details</h2>
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">Name</label>
            <input
              required value={form.name}
              onChange={e => setForm({ ...form, name: e.target.value })}
              className="w-full px-3 py-2 border border-zinc-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-white/20"
            />
          </div>
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="flex-1 px-3 py-2 border border-zinc-700 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-white/20"
              />
            </div>
          </div>
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 border border-zinc-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-white/20 resize-none"
            />
          </div>
        </div>
        ...
        {/* Duration & buffers */}
        <div className="bg-zinc-950 rounded-xl border border-zinc-800 p-5 space-y-4">
          <h2 className="text-sm font-semibold text-zinc-300 uppercase tracking-wide">Duration & buffers</h2>
        ...
        <div className="bg-zinc-950 rounded-xl border border-zinc-800 p-5 space-y-3">
          <h2 className="text-sm font-semibold text-zinc-300 uppercase tracking-wide">Weekly availability</h2>
              ...
              return (
                <div key={key} className="flex items-center gap-3">
                  <button
                    ...
                    className={`w-24 text-left text-sm py-1.5 px-2 rounded-md transition-colors font-medium ${enabled ? 'text-white bg-zinc-800' : 'text-zinc-600 hover:text-zinc-400'}`}
                  >
                    {label.slice(0, 3)}
                  </button>
                  {enabled && slot ? (
                    <div className="flex items-center gap-2">
                      <input type="time" value={slot.start} onChange={e => updateDaySlot(key, 'start', e.target.value)}
                        className="px-2 py-1.5 border border-zinc-700 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-white/20" />
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-5 py-2.5 bg-white hover:bg-zinc-100 disabled:opacity-60 text-white text-sm font-medium rounded-lg transition-colors"
          >
            {saving ? 'Saving...' : 'Save changes'}
          </button>
          <Link href="/dashboard/links" className="px-5 py-2.5 text-sm font-medium text-zinc-300 hover:bg-zinc-800 rounded-lg transition-colors">
            Cancel
          </Link>
        </div>
      </form>
