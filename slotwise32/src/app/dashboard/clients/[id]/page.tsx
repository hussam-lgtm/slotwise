    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-zinc-800 flex items-center justify-center text-lg font-semibold text-white">
            {client.name[0].toUpperCase()}
          </div>
          <div>
            <h1 className="text-2xl font-semibold text-white">{client.name}</h1>
            <p className="text-sm text-zinc-500">{client.company ?? client.email ?? 'No contact info'}</p>
          </div>
        </div>
        <Link
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {/* Calendar status */}
        <div className="col-span-1 bg-zinc-950 rounded-xl border border-zinc-800 p-4">
          <div className="flex items-center gap-2 mb-2">
            <Calendar size={15} className="text-zinc-500" />
            <span className="text-sm font-medium text-zinc-300">Calendar</span>
          </div>
          {calConn ? (
          ...
          {!calConn && (
            <Link
              ...
              className="mt-2 text-xs text-white hover:text-white font-medium"
            >
              Connect calendar →
            </Link>
          )}
        ...
        <div className="bg-zinc-950 rounded-xl border border-zinc-800 p-4">
          <p className="text-sm text-zinc-500 mb-1">Booking links</p>
          <p className="text-2xl font-semibold text-white">{client.booking_links?.length ?? 0}</p>
        </div>
        <div className="bg-zinc-950 rounded-xl border border-zinc-800 p-4">
      ...
      <div className="bg-zinc-950 rounded-xl border border-zinc-800">
        <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
          <h2 className="font-medium text-white">Booking links</h2>
          <Link
            href={`/dashboard/links/new?client=${id}`}
        ...
        {client.booking_links && client.booking_links.length > 0 ? (
          ...
          <div className="px-5 py-10 text-center text-sm text-zinc-600">
            ...
            <Link href={`/dashboard/links/new?client=${id}`} className="text-white hover:text-white">
              Create one
            </Link>
          </div>
        )}
